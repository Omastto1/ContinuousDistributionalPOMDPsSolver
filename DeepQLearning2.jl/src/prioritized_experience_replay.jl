...
mutable struct PrioritizedReplayBuffer{N<:Integer, T<:AbstractFloat,CI,Q,A<:AbstractArray{Matrix{T}}}
    max_size::Int64
    batch_size::Int64
    rng::AbstractRNG
    α::Float32
...
function PrioritizedReplayBuffer(env::AbstractEnv,
    ...
    o = observe(env)
    # print(size(o))
    # print(size(Mill.data(o)))
    # s_dim = size(Mill.data(o))
    s_dim = size(o)
    # s_dim = length(values(o))
    # println("size: $(length(values(o)))")
    # println("Observation: $(o)")
    # println("Type: $(typeof(o))")
    # println("size: $(size(Mill.data(o)))")
    # println("size: $(size(o))")
    # println("GOOD TYPE: $(typeof(o) <: (AbstractArray{<:Real,N} where N))")
    # experience = Vector{DQExperience{Int32, Float32, typeof(Mill.data(o))}}(undef, max_size)
    experience = Vector{DQExperience{Int32, Float32, typeof(o)}}(undef, max_size)
    priorities = Vector{Float32}(undef, max_size)
    # @show experience

    # asd()
    # _s_batch = zeros(Float32, s_dim[1], 9, batch_size)
    _s_batch = Vector{Matrix{Float32}}(undef, batch_size)
    _a_batch = [CartesianIndex(0,0) for i=1:batch_size]
    _r_batch = zeros(Float32, batch_size)
    # _sp_batch = zeros(Float32, s_dim[1], 9, batch_size)
    _sp_batch = Vector{Matrix{Float32}}(undef, batch_size)
...
function get_batch(r::PrioritizedReplayBuffer, sample_indices::Vector{Int64})
    ...
    # @show cumsum(last.(size.(r._sp_batch)))
    # @show r._sp_batch
    # @show size(hcat(r._sp_batch...))
    return hcat(r._s_batch...), cumsum(last.(size.(r._s_batch))), r._a_batch, r._r_batch, hcat(r._sp_batch...), cumsum(last.(size.(r._sp_batch))), r._done_batch, sample_indices, weights
end
