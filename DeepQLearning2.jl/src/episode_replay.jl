...
function generate_episode(env::AbstractEnv, action_indices; max_steps::Int64 = 100)
    ...
    # start simulation
    reset!(env)
    belief = observe(env)    # prevedu observaci na array - zkombinuju s a o informaci
    done = false
    step = 1
    while !done && step < max_steps
        action = rand(actions(env))
        ai = action_indices[action]
        rew = act!(env, action)#, belief) # provedu akci a aktualizuju stav i observaci
        bp = observe(env) # prevedu observaci na array - zkombinuju s a o informaci
        done = terminated(env)
