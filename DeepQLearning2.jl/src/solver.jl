...
function POMDPs.solve(solver::DeepQLearningSolver, problem::MDP)
    ...
    # @show solver.qnetwork.im.m.weight
...
function POMDPs.solve(solver::DeepQLearningSolver, env::AbstractEnv)
    ...
    # @show policy.qnetwork.im.m.weight
...
function dqn_train!(solver::DeepQLearningSolver, env::AbstractEnv, policy::AbstractNNPolicy, replay)
    ...
    solver.verbose ? println("Setting up the solver") : ()
    ...
    for t=1:solver.max_steps
        # @show belief
        # @show ArrayNode(belief, size(belief)[2])
        act = action(solver.exploration_policy, policy, t, belief)
        # act = action(solver.exploration_policy, policy, t, ArrayNode(belief, size(belief)[2]))
        ai = action_indices[act]
        rew = act!(env, act)#, belief)
        bp = observe(env)
        done = terminated(env)
        exp = DQExperience(belief, ai, Float32(rew), bp, done)
        # solver.verbose ? println("recording experience: $(exp)") : ()
        ...
        if t%solver.train_freq == 0
            # solver.verbose && println("training batch")
            # println("FIX THIS")
...
function batch_train!(solver::DeepQLearningSolver,
    ...
    s_batch, s_csum, a_batch, r_batch, sp_batch, sp_csum, done_batch, indices, importance_weights = sample(replay)

    # MILL BAGNODE SETUP
    sps_csum = copy(sp_csum)
    sps_csum = (pushfirst!(sps_csum, 0) .+ 1)[1:end-1]
    bags = (:).(sps_csum, sp_csum)
    sp_batch = BagNode(ArrayNode(sp_batch), bags)
    # @show bags
    # @show size(Mill.data(Mill.data(sp_batch)))

    # @show Mill.data(sp_batch[end])
    ...
    if solver.double_q
        ...
        # @show qp_values
        best_a = [CartesianIndex(argmax(Mill.data(qp_values)[:, i]), i) for i=1:solver.batch_size]
        # @show target_q_values
        q_sp_max = Mill.data(target_q_values)[best_a]
        # println("PROC JSEM TADY \n\n\n\n")
        # @show Mill.data(target_q_values)
        # @show Mill.data(target_q_values)[best_a]
    else
    ...
    ss_csum = copy(s_csum)
    ss_csum = (pushfirst!(ss_csum, 0) .+ 1)[1:end-1]
    bags = (:).(ss_csum, s_csum)
    s_batch = BagNode(ArrayNode(s_batch), bags)

    gs = Flux.gradient(p) do
        # println("gradient")
        # @show active_q.im.m.weight
        # @show first(active_q.a.fs).ψ
        # @show active_q.bm.m.weight
        # @show s_batch
        # @show Mill.data(Mill.data(s_batch)[1])
        # @show active_q
        q_values = active_q(s_batch)
        # @show(q_values)
        # @show q_targets
        q_sa = Mill.data(q_values)[a_batch]
        # @show q_sa
